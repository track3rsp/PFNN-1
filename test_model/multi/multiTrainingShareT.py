...
def LoadDog(filename):
    ...
    Traj_in = np.float32(np.zeros((A.shape[0],np.int(num_trajectory*4))))
    Traj_in_start = offset+jointNeurons
    for i in range(np.int(num_trajectory)):
        Traj_in[:,i*4:(i+1)*4] = B[:,[Traj_in_start, Traj_in_start+2, Traj_in_start+3, Traj_in_start+5]]
        Traj_in_start += 6 
    #input 
    X = np.concatenate(
            (
                    Traj_in, #trajectory pos, dir, hei, style of B
    ...
    inshare_size   = np.int(num_trajectory * 4 )
    outshare_size  = np.int(num_trajectory/2 * 4)
    
    print("DogData is processed")
    return input_x, input_y, input_size, output_size, number_example, inshare_size, outshare_size
    

input_human, output_human, inputSize_human, outputSize_human, size_human = LoadHuman('database.npz')
input_dog,   output_dog,   inputSize_dog,   outputSize_dog,   size_dog, inshare_size, outshare_size   = LoadDog('Data.txt')
...
P0_human  = PFNNParameter((nslices, 512, inputSize_human-1-inshare_size), rng, phase, 'wb0_human')
P0_dog    = PFNNParameter((nslices, 512, inputSize_dog-1-inshare_size), rng, phase, 'wb0_dog')
P0_share  = PFNNParameter((nslices, 512, inshare_size), rng, phase, 'wb_share')

P1        = PFNNParameter((nslices, 512, 512), rng, phase, 'wb1')

P2_human  = PFNNParameter((nslices, outputSize_human-outshare_size, 512), rng, phase, 'wb2_human')
P2_dog    = PFNNParameter((nslices, outputSize_dog-outshare_size, 512), rng, phase, 'wb2_dog')
P2_share  = PFNNParameter((nslices, outshare_size, 512), rng, phase, 'wb2_share')


"""weights and parameter of nn"""
W0 = tf.cond(tf.less(tensor_1, flag), 
             lambda: tf.concat([P0_share.weight,P0_human.weight],axis = -1),
             lambda: tf.concat([P0_share.weight,P0_dog.weight],  axis = -1)
             )

W1 = P1.weight

W2 = tf.cond(tf.less(tensor_1, flag), 
             lambda: tf.concat([P2_human.weight[:,0:8,:],P2_share.weight, P2_human.weight[:,8:,:]],axis = 1),
             lambda: tf.concat([P2_share.weight, P2_dog.weight],  axis = 1)
             )

b0 = tf.cond(tf.less(tensor_1, flag), 
             lambda: P0_share.bias+P0_human.bias,
             lambda: P0_share.bias+P0_dog.bias
             )

b1 = P1.bias

b2 = tf.cond(tf.less(tensor_1, flag), 
             lambda: tf.concat([P2_human.bias[...,0:8],P2_share.bias, P2_human.bias[...,8:]],axis = -1),
             lambda: tf.concat([P2_share.bias,P2_dog.bias],axis = -1),
             )
...
regularization = tf.cond(tf.less(tensor_1, flag),  
                         lambda: PFNN.regularization_penalty((P0_human.alpha, P0_share.alpha, P1.alpha, P2_human.alpha, P2_share.alpha),0.01),
                         lambda: PFNN.regularization_penalty((P0_dog.alpha,   P0_share.alpha, P1.alpha, P2_dog.alpha,   P2_share.alpha),0.01)
                         )
...
for epoch in range(training_epochs):
    ...
    PFNN.save_network((sess.run(tf.concat([P0_share.alpha, P0_dog.alpha], axis = -1)),sess.run(P1.alpha),sess.run(tf.concat([P2_share.alpha,P2_dog.alpha],axis=-1))),
                      (sess.run(P0_share.beta + P0_dog.beta), sess.run(P1.beta), sess.run(tf.concat([P2_share.beta, P2_dog.beta], axis=-1))),
                      50,
                      './result/dogNN'
                      )
    PFNN.save_network((sess.run(tf.concat([P0_share.alpha, P0_human.alpha],axis = -1)),sess.run(P1.alpha),sess.run(tf.concat([P2_human.alpha[:,0:8,:],P2_share.alpha,P2_human.alpha[:,8:,:]],axis=1))),
                      (sess.run(P0_share.beta + P0_human.beta), sess.run(P1.beta), sess.run(tf.concat([P2_human.beta[...,0:8], P2_share.beta, P2_human.beta[...,8:]],axis=-1))),
                      50,
                      './result/humanNN'
                      )
