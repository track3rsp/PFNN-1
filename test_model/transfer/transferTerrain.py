#pretrained the human and fix the hidden layer.
...
#get the terrain index for multitask/transfer learning
...
#transfer terrain
for i in range(terrainNeurons):
    index_t = index_terrain[i]
    update = tf.assign(P0.alpha[...,index_t], terrainAlpha[...,i])
...
P1.beta.trainable = False

#optimizer, learning rate 0.0001
learning_rate = 0.0001
#optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(loss_regularization, var_list=tf_train)
...
amount_test = 0.1
num_testBatch  = np.int(total_batch * amount_test)
