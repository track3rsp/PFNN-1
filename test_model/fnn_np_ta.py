...
"""input of nn"""
X_nn = tf.placeholder(tf.float32, [None, input_size], name='x-input')  #?*342+1
...
# W1: 513*512 
W1 = tf.get_variable("W1", shape=[513, 512], initializer=tf.contrib.layers.xavier_initializer())
b1 = tf.Variable(initial_beta((1, 512)), name='b1') #b1: 1*512

# W2: 513*311
W2 = tf.get_variable("W2", shape=[513, output_size], initializer=tf.contrib.layers.xavier_initializer())
b2 = tf.Variable(initial_beta((1, output_size)), name='b2') #b2: 1*311

#will be added into layerd
phase_tensor = X_nn[:,-1][:,np.newaxis]

#structure of nn
H0 = X_nn        #input of nn     dims:  ?*343
H0 = tf.nn.dropout(H0, keep_prob=keep_prob)

H1 = tf.matmul(H0, W0) + b0      #dims:  ?*343 mul 343*512 = ?*512
...
H1 = tf.concat([H1, phase_tensor], axis = 1) #?*513


H2 = tf.matmul(H1, W1) + b1       #dims: ?*512 mul 513*512 = ?*512
...
H2 = tf.concat([H2, phase_tensor], axis = 1) #?*513

H3 = tf.matmul(H2, W2) + b2       #dims: ?*513 mul 513*311 =?*311
