...
P2        = PFNNParameter((nslices, 512, 512), rng, phase, 'wb2')
P3_human  = PFNNParameter((nslices, outputSize_human, 512), rng, phase, 'wb3_human')
P3_dog    = PFNNParameter((nslices, outputSize_dog, 512), rng, phase, 'wb3_dog')
...
W3 = tf.cond(tf.less(tensor_1, flag), 
             lambda: P3_human.weight,
             lambda: P3_dog.weight
             )
...
b3 = tf.cond(tf.less(tensor_1, flag), 
             lambda: P3_human.bias,
             lambda: P3_dog.bias
             )
...
cost = tf.reduce_mean(tf.square(Y - H4))+regularization
...
for epoch in range(training_epochs):
    ...
    PFNN.save_network((sess.run(P0_dog.alpha),sess.run(P1.alpha),sess.run(P2.alpha), sess.run(P3_dog.alpha)),
                      (sess.run(P0_dog.beta), sess.run(P1.beta), sess.run(P2.beta),  sess.run(P3_dog.beta)),
                      50,
                      './result/dognn'
                      )
    PFNN.save_network((sess.run(P0_human.alpha),sess.run(P1.alpha),sess.run(P2.alpha), sess.run(P3_human.alpha)),
                      (sess.run(P0_human.beta), sess.run(P1.beta), sess.run(P2.beta),  sess.run(P3_human.beta)),
                      50,
                      './result/humanNN'
                      )    
